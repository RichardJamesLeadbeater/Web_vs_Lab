...
let expName = 'Spatial2AFC_homevslab';  // from the Builder filename that created this script
let expInfo = {'': ''};
...
function updateInfo() {
  ...
  psychoJS.setRedirectUrls('https://run.pavlovia.org/RichardL/spatial2afc_homevslab', 'https://run.pavlovia.org/RichardL/spatial2afc_homevslab');
...
function experimentInit() {
  ...
  text_begin = new visual.TextStim({
    win: psychoJS.window,
    name: 'text_begin',
    text: "ALWAYS MAINTAIN FIXATION ON THE CENTRAL DOT.\n\npress 'F' or 'J' to begin",
    font: 'Arial',
    units: 'norm', 
    pos: [0, (- 0.7)], height: 0.075,  wrapWidth: 2, ori: 0,
    color: new util.Color('white'),  opacity: 1,
  ...
  tone_interval = new sound.Sound({
    win: psychoJS.window,
    value: '500',
    secs: 0.2,
    });
  tone_interval.setVolume(0.1);
...
function custom_codeRoutineBegin(snapshot) {
  return function () {
    ...
    for (const thisComponent of custom_codeComponents)
...
function targetRoutineEachFrame(snapshot) {
  return function () {
    ...
    if (t >= 0.0 && key_target.status === PsychoJS.Status.NOT_STARTED) {
      ...
      // keyboard checking is just starting
      key_target.clock.reset();
      key_target.start();
      key_target.clearEvents();
    }
...
function trialRoutineBegin(snapshot) {
  return function () {
    ...
    // keep track of which components have finished
    trialComponents = [];
    trialComponents.push(tone_interval);
...
function trialRoutineEachFrame(snapshot) {
  return function () {
    ...
    frameN = frameN + 1;// number of completed frames (so 0 is the first frame)
    // update/draw components on each frame
    // start/stop tone_interval
    if (t >= stim["interval_tstart"] && tone_interval.status === PsychoJS.Status.NOT_STARTED) {
      // keep track of start time/frame for later
      tone_interval.tStart = t;  // (not accounting for frame time here)
      tone_interval.frameNStart = frameN;  // exact frame index
      
      psychoJS.window.callOnFlip(function(){ tone_interval.play(); });  // screen flip
      tone_interval.status = PsychoJS.Status.STARTED;
    }
    frameRemains = stim["interval_tstart"] + 0.2 - psychoJS.window.monitorFramePeriod * 0.75;  // most of one frame period left
    if ((tone_interval.status === PsychoJS.Status.STARTED || tone_interval.status === PsychoJS.Status.FINISHED) && t >= frameRemains) {
      if (0.2 > 0.5) {  tone_interval.stop();  // stop the sound (if longer than duration)
        tone_interval.status = PsychoJS.Status.FINISHED;
      }
    }
    
    // *grating_target* updates
    if (t >= stim["interval_tstart"] && grating_target.status === PsychoJS.Status.NOT_STARTED) {
      // keep track of start time/frame for later
    ...
    frameRemains = stim["interval_tstart"] + stim["duration"] - psychoJS.window.monitorFramePeriod * 0.75;  // most of one frame period left
    ...
    if (t >= stim["interval_tstart"] && grating_foil.status === PsychoJS.Status.NOT_STARTED) {
      // keep track of start time/frame for later
    ...
    if (t >= dot_tstart && dot_fixation.status === PsychoJS.Status.NOT_STARTED) {
      // keep track of start time/frame for later
    ...
    // *key_trial* updates
    if (t >= (stim["interval_tstart"] + stim["duration"]) && key_trial.status === PsychoJS.Status.NOT_STARTED) {
      // keep track of start time/frame for later
...
function trialRoutineEnd(snapshot) {
  return function () {
    ...
    tone_interval.stop();  // ensure sound has stopped at end of routine
